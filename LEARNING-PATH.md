...
1. **Новичок:** Начни с разделов 1-3 (фундамент, контуры системы, репозитории, протоколы). Это ~3 часа.
...
| 22 | Platform ≠ IWE Template ≠ Personal IWE | 4 контура системы (§ 1.4) |

**Где изучить:**
- [memory/hard-distinctions.md](./memory/hard-distinctions.md) — все 22 пары с примерами и тестами

### 1.4. Контуры системы (Platform Contours)

IWE существует не изолированно — она часть 4-контурной системы. Понимание контуров помогает видеть, где ты находишься и что тебе доступно.
...
**Путь пользователя через контуры:**
...
**Правило IntegrationGate:** Перед добавлением нового инструмента или агента в свой IWE — определи: (1) тип (инструмент/агент), (2) контур (L2/L3/L4), (3) роли, (4) продукты, (5) процессы. Это помогает не добавлять лишнего и понимать, как новый компонент встраивается.

**Где изучить:**
- [ONTOLOGY.md](./ONTOLOGY.md) § «Контуры системы» — компактное описание
- [DS-ecosystem-development/11-platform-contours.md](https://github.com/TserenTserenov/DS-ecosystem-development) — полная архитектурная модель с маппингом на ядра
- [memory/hard-distinctions.md](./memory/hard-distinctions.md) § 23 — тесты различения контуров
...
**Маппинг тиров на контуры системы (§ 1.4):**
...
| В каком контуре я нахожусь? | L4 (Personal IWE), если T4+. L2 (Platform), если T1-T3 | § 1.4 |
| Куда добавить новый инструмент? | Определи контур: L2 (платформа) / L3 (шаблон) / L4 (личный) | [CLAUDE.md](./CLAUDE.md) Pre-action Gates (IntegrationGate) |
...
1. Прочитай контуры системы (§ 1.4) — 10 мин (пойми, где ты в системе)
