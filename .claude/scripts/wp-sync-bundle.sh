...
registry_status() {
  ...
  # Префикс `WP-` в первой ячейке ("| WP-10 |") не матчился, хотя ROW_RE
  # в build-active-wp.py принимает его — реестр пилота пишет номер с
  # префиксом, canary падал с "_не в реестре_" для любого РП.
  local regex="^\|[[:space:]]*(~~)?(\*\*)?(WP-)?${num}(\*\*)?(~~)?[^0-9|]*[[:space:]]*\|"
  local match_count
